...
a single derivation is in use, or ``jet notation'', which I adopt
in this paper, where multiple derivations are specified (the PDE case)
and they are denoted by subscripted letters, i.e, $\Psi_x$
...
to figure out that $\Psi$ is a multiple of the exponential of $x$.
...
our solution to be in $C^2(\mathbf{R})$, then we try parameterize a subset of
$C^2(\mathbf{R})$ using some number of complex variables, say twenty, so that our resulting
equations are polynomials in $\mathbf{C}[a,..,t]$ and their common
...
The first requirement is a natural one within the context of differential algebra,
...
The next two requirements ensure that we can ultimately reduce the problem
...
my sole objects of study are ansatzen constructed from a finite number of holonomic
extensions.

A holonomic extension of 

Note: This definition of holonomic extension may not match with the D-module literature.  Check this.

I further restrict to linear ODEs because their theory
...
Its structure can also be described using differential polynomials:
...
a linear variable $v$.  Strictly speaking, a $v_0$ constant term
...
Next, we essentially introduce a new derivation with respect to $v$.
...
\begin{enumerate}
\item Select an ansatz that defines a differential space parameterized by constants (see next section)
...
Ansatz 16.6&Helium         &Differential Elimination &Manual                &10 min\\
           ...
           &               &Solve System of Eqns     &Levenberg-Marquardt   &work in progress\\
\end{longtable}
...
when these ansatzen might yield solutions.
...
the software tools necessary to check ansatz 16.6
