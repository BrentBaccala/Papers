...
To work with my limited reduction algorithm, there are some additional requirements:

\begin{itemize}
\item new indeterminates must be either constant, algebraic, or ODE
\item the deriviations of constants, by definition, are zero
\item algebraic elements must be specified using an minimum polynomial, which may includes constants
\item derivations of algebraic elements are completely defined by their minimum polynomial
\item ODE indeterminates are paired.  Each ODE extension is defined using two indeterminates,
the dependent and the independant variable.  The dependent variable is specified using a minimum differential
equation which only involves derivatives of the dependent variable with respect to the independent variable.
\item derivations of the independant variable are completely defined in terms of derivations of the
dependent variable using the chain rule.  If $\Psi$ is the independent variable and $v$ is the dependent
variable, then $\Psi_x = \Psi_v v_x$
\item the new inderminates must be specified in differential triangular form, or as a tower of extensions
\end{itemize}

Algorithm:

\begin{itemize}
\item form a differentially triangular set that describes the ansatz
\item reduce (which reduction?) the PDE modulo the differentially triangular set
\item collect like terms
\item form a system of polynomial equations in the constants
\end{itemize}

triangular - it's like a tower; each equation introduces a new variable (i.e, leaders are different),
  but in differential algebra, it could introduce new derivations of the same variable
     $v_x$ = nothing involving v
     $v_y$ = nothing involving v (this is fine, they're pairwise partially reduced, since no proper derivative of $v_x$ or $v_y$ appears in the other)
     $v_xx$ = ... (this is a problem; $v_x$ now has a proper derivative here)

What does it take for my cancellation scheme to work?
   - it must be a tower, more or less, so triangular (more or less)
   - each algebraic element must be introduced with a minimal polynomial, that will be used to cancel the leader's power and higher powers of leader
   - each ODE element is introduced with a differential polynomial w.r.t. a new variable v
   - v's first derivatives are all specified using the chain rule

How hard is it to introduce a new independent variable?
   - an equation written using derivatives w.r.t. the new independent can be written as $v_a = v_x x_a$

How to introduce a new ODE?

\begin{itemize}
\item We wish to avoid introducing new derivations.
\item We introduce new symbols, $\Psi$, $\Psi'$, $\Psi'', v$
\item $\Psi$ is described by a minimal differential polynomial involving only $\Psi''$, $\Psi'$, $\Psi$, and $v$.
\item We define the derivations of $\Psi$ and $\Psi'$ using the chain rule: $\Psi_x = \Psi' v_x$; $\Psi'_x = \Psi'' v_x$
\item The derivations of $\Psi''$ are now completely defined by its minimal differential polynomial
...
For our tower, there are two types of admissible extensions: algebraic and ODE.
...
A {\it ODE extension} of differential field is a transcendental (i.e, non-algebraic)
...
We expect to also consider coefficients selected from an extension (algebraic or ODE)
...
A {\it ODE tower} is a differential field extension
constructed from a finite number of algebraic and/or ODE extensions.

Our ansatzen are formed from ODE towers with degree bounds on all
...
\tikzstyle{degree}=[]
  ...
  \begin{tikzpicture}[remember picture]
    ...
    \node (ODE extension 2) [element, below=of ODE extension, text width=200pt] {ODE extension\\generated by adjoining ODE element\\and forming the ring so generated};
...
Ansatz 5 is a second-order ODE element with linear coefficients and a linear variable.
...
involving $v_i$, $d_i$, $m_i$, $n_i$ and $E$ that, if satisfied,
...
correct operation of the projection step is predicated on nothing more than separating
...
A second order ODE extension in the coefficient ring.
...
An algebraic extension nested below an ODE extension.

\begin{tabular}{lccc}
 & \multicolumn{3}{c}{\bf degree bounds} \\
             & algebraic root & $v$ & ODE coefficients \\
Ansatz 16    & 1  & 1  & 1 \\
Ansatz 16.1  & 1  & 2  & 1 \\
Ansatz 16.2  & 1  & 1  & 2 \\
Ansatz 16.3  & 2  & 1  & 1 \\
